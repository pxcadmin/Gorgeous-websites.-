export const siteUrl = "https://patientexperienceconsulting.com";
export const contactEmail = "impact@patientexpepatientexperienceconsulting.com";
export const contactPhone = "(941) 239-2309";
export const navigation = [
  { label: "About Jennifer", to: "/about-jennifer-radu" },
  { label: "Services", to: "/services" },
  { label: "Results", to: "/results" },
  { label: "Resources", to: "/free-resources" },
  { label: "Contact", to: "/contact" },
];
export const frameworks = [
  {
    stage: "DIAGNOSE",
    name: "VIBE-Coded Practice Blueprint™",
    text: "Maps where your practice is losing time, money, energy, and patient trust while revealing where AI can responsibly create value.",
    outcome:
      "Clarity on what is broken, what matters most, and where to start.",
  },
  {
    stage: "STRATEGIZE",
    name: "IMPACT System™",
    text: "Aligns leadership, culture, patient experience, workflow priorities, and AI opportunity into one focused transformation roadmap.",
    outcome: "A plan built around the actual needs of your practice.",
  },
  {
    stage: "DEPLOY",
    name: "Patient eXperience Pyramid Puzzle™",
    text: "Turns strategy into structured execution so change happens in the right order, with the right systems and team buy-in.",
    outcome: "Execution without chaos, confusion, or scattered priorities.",
  },
  {
    stage: "SUSTAIN",
    name: "PX Cycle™ – AI-Fueled, People-Powered",
    text: "Embeds feedback loops, KPI reviews, training rhythms, and optimization systems so results compound instead of fading.",
    outcome: "Lasting transformation instead of short-term improvement.",
  },
];
export const offers = [
  {
    name: "Practice Clarity Assessment™",
    best: "Practices that need diagnosis, clarity, and AI readiness insight before bigger decisions.",
    price: "from $997",
    includes: [
      "Practice diagnosis",
      "AI Readiness Score",
      "Lost Revenue + Wasted Time Report",
      "AI Integration Opportunity Map",
    ],
    to: "/services/practice-clarity-assessment",
  },
  {
    name: "IMPACT Blueprint™",
    best: "Practices ready for strategic intervention, leadership alignment, and early AI implementation.",
    price: "$8,500/mo",
    includes: [
      "3–4 month fCAIOX engagement",
      "Strategic transformation roadmap",
      "Leadership and culture intervention",
      "2–4 custom AI builds",
    ],
    to: "/services/impact-blueprint",
  },
  {
    name: "EXCEL Transformation™ Partnership",
    best: "Practices that want an embedded, long-term partner to operationalize change.",
    price: "$13,500/mo",
    includes: [
      "12-month fCAIOX partnership",
      "Custom AI deployment expansion",
      "Governance systems",
      "Operational redesign implementation",
    ],
    to: "/services/excel-transformation",
  },
  {
    name: "Practice Professionals Playground",
    best: "Leaders who want monthly training, office hours, tools, templates, and ongoing AI learning.",
    price: "$197/mo or $1,997/yr",
    includes: [
      "Monthly live learning",
      "Early access to tools",
      "Office hours",
      "Peer implementation community",
    ],
    to: "/services/practice-professionals-playground",
  },
  {
    name: "AI Blueprint Day™",
    best: "Practices, associations, and events that need a focused AI roadmap workshop.",
    price: "from $4,000",
    includes: [
      "Customized AI workshop",
      "Roadmap creation",
      "Implementation guidance",
      "Leader and team training",
    ],
    to: "/ai-blueprint-day",
  },
];
export const faqs = [
  [
    "What types of practices does PXC serve?",
    "Patient eXperience Consulting® serves established independent and group medical practices, typically 2–20 providers, facing burnout, turnover, bottlenecks, stagnant growth, or low patient experience metrics.",
  ],
  [
    "How is PXC different from a traditional consultant?",
    "PXC combines healthcare operations, culture, patient experience, custom AI builds, governance, and founder-led implementation inside one integrated transformation model.",
  ],
  [
    "Will AI replace my team?",
    "No. PXC uses AI to remove repetitive administrative drag while making people more capable, supported, and present for patients.",
  ],
  [
    "How quickly can we see results?",
    "Most practices identify quick wins in the first 30 days and build measurable momentum through a 90–180 day transformation window.",
  ],
  [
    "Do we need to be tech-savvy?",
    "Not at all. Jennifer translates complex tools into practical workflows, trains your team, and guides adoption at a sustainable pace.",
  ],
  [
    "What is the first step?",
    "Book a free 30-minute Practice Clarity Strategy Session. We identify your most expensive friction point and recommend the right starting path.",
  ],
];
export const posts = [
  {
    category: "AI Governance",
    title: "The Practice Leader’s Guide to Safe AI Adoption",
    excerpt:
      "AI implementation does not begin with a tool. It begins with a governed decision about the work your team needs back.",
    date: "May 14, 2025",
  },
  {
    category: "Patient Experience",
    title: "Why Your Patient Experience Problem May Be an Operations Problem",
    excerpt:
      "The patient sees the last handoff. Your team lives every broken one before it.",
    date: "May 7, 2025",
  },
  {
    category: "Practice Growth",
    title: "The 30-Day Practice Stability Reset",
    excerpt:
      "A focused reset helps leaders turn scattered frustration into visible priorities and accountable next steps.",
    date: "April 29, 2025",
  },
];
